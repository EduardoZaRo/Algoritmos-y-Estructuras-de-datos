...
int main(){
	...
	do{
		...
		printf("La cadena a codificar es %s con un desplazamiento de %d caracteres\n\n",input, desplazamiento);
...
void desplazar(char* cadena, int desplazamiento){
	int i, j;
	char a[desplazamiento];//En este char se guardan los valores que se pierden al desplazar 
	...
	for(i = 0; i < strlen(cadena);i++){
		for(j = 0; j < strlen(alfabeto);j++){
			if(cadena[i] == alfabeto[j]){
				printf("%d %d    %c     %c      %c\n", i, j, cadena[i], alfabeto[j],codificador[j]);
